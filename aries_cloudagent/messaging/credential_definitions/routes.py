...
async def credential_definitions_send_credential_definition(request: web.BaseRequest):
    ...
    if support_revocation and novel:
        ...
        try:
            ...
            raise web.HTTPBadRequest(reason=e.message) from e
        await shield(registry_record.generate_registry(context.profile))
        try:
            await registry_record.set_tails_file_public_uri(
                context.profile, f"{tails_base_url}/{registry_record.revoc_reg_id}"
            )
            await registry_record.send_def(context.profile)
            await registry_record.send_entry(context.profile)
            ...
            ensure_future(
                pending_registry_record.stage_pending_registry(
                    context.profile, max_attempts=16
                )
            )

            tails_server = context.inject(BaseTailsServer)
